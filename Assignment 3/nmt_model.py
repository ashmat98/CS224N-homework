...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab : Vocab, dropout_rate=0.2):
        ...
        ###    (Conv1d layer with kernel size 2, input and output channels = embed_size,padding = same to preserve output shape )
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X_cnn = torch.permute(X_cnn, (2,0,1)) # (src_len, b, e)
        ...
        # (src_len, b, 2*h)  (2,b,h)       (2,b,h)
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(X_packed) 

        enc_hiddens, _ = pad_packed_sequence(enc_hiddens_packed) # (src_len, b, 2*h)

        enc_hiddens = torch.permute(enc_hiddens, (1,0,2))  # (b, src_len, 2*h)
        ...
        def concat_helper(tns):
            """
                (2, b, h) to (b, 2*h)
            """
            tns_permuted = tns.permute(1, 0, 2)
            return tns_permuted.reshape(tns_permuted.shape[0], -1)

        init_decoder_hidden = self.h_projection(concat_helper(last_hidden)) # (b*h)
        init_decoder_cell = self.c_projection(concat_helper(last_cell))     # (b*h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print("dec_state:",dec_state[0].shape, dec_state[1].shape,)
        
        dec_new_state = self.decoder(Ybar_t, dec_state)
        
        ###     2. Split dec_state into its two parts (dec_hidden, dec_cell)
        h_t, c_t = dec_new_state
        h_t : torch.Tensor = h_t # (b, h)
        ...
        # print("h_t:", h_t.shape)
        #                  (b, src_len, h)    (b, h, 1)
        e_t = torch.bmm(enc_hiddens_proj, h_t.unsqueeze(2))  # (b, src_len, 1)
        ...
        # print("a_t:", a_t.shape)
        # print("h_t:", h_t.shape)
        U_t = torch.cat([a_t, h_t], dim = 1) # (b, 3h)
        ...
        combined_output = O_t
        return dec_new_state, combined_output, e_t
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # log probabilities over target words   (b, vocab_len)
            log_p_t = F.log_softmax(self.target_vocab_projection(att_t), dim=-1)

            live_hyp_num = beam_size - len(completed_hypotheses)
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_p_t).view(-1) # b*vocab_len
